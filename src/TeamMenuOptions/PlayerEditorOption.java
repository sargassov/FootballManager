...
public class PlayerEditorOption implements TeamMenuOptionsInterface {
    ...
    public String guessNumber (Tournament rfpl){
        ...
        String line = "";
        ArrayList<Short> closedNumbers = (ArrayList<Short>) rfpl.my_team.list.stream().map(p->p.number).sorted()
        ...
        while(!openNumbers.contains(num)){
            out.print(Corrector.GetSpace(48) + "You can take only free unique number: ");
            int lineCounter = 0;
            for(Short sh : openNumbers){
                if(lineCounter % 10 == 0) out.print("\n" + Corrector.GetSpace(50) + sh + " ");
                else out.print(sh + " ");
                lineCounter++;
            }
    ...
    private void playerCreator (Tournament rfpl, String line){
        ...
        out.println("\t\t\t\tYou were created a new player " + player.name + ". Price for this player is: " + player.price);
        if(rfpl.my_team.wealth >= player.price){
        ...
        else out.println("You can't buy this player. Not enough credits");
        return;
    }
