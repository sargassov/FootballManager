...
public class VisualCalendarTable extends Table implements Data {
    ...
    public void toPrint(Tournament rfpl) {
        ...
        for (int x = 0; x < cloneVisualCalendarTable.fields.size(); x++) {
            ...
            out.println(cloneVisualCalendarTable.fields.get(x));
        }
    }
    ...
    private  static void editEvent(ArrayList<Day> days, int monthDayCounter, String[] cutString, int y, Tournament rfpl){
        if(days.get(monthDayCounter) instanceof DayMatch){
            ...
            if(match.home.equals(rfpl.my_team.name)){
                if(match.away.length() > 11){ cutString[y] = Corrector.wordToCenter(match.away.substring(0,8) + " A", 11);}
                else {cutString[y] = Corrector.wordToCenter(match.away + " H", 11);}
            }
            else if(match.away.equals(rfpl.my_team.name)){
                if(match.home.length() > 11) {cutString[y] = Corrector.wordToCenter(match.home.substring(0,8) + " H", 11);}
                else {cutString[y] = Corrector.wordToCenter(match.home + " A", 11);}
            }
        }
        else{
